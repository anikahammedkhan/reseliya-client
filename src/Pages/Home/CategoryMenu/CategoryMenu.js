...
const CategoryMenu = () => {
    ...
    useEffect(() => {
        fetch('https://reseliya-server.vercel.app/categories')
            .then(res => res.json())
    ...
    return (
        <div className='my-12'>
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 my-8 px-5">
                {
                    categories.map(category => <Link to={`/category/${category.categoryName}`} className="card card-compact bg-base-100 shadow-xl" key={category._id}>
